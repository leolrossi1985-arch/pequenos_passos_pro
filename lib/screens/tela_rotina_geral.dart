...
import '../services/resumo_service.dart'; 
...
class _TelaRotinaGeralState extends State<TelaRotinaGeral> with TickerProviderStateMixin {
  ...
  // --- LÓGICA DE NEGÓCIO ---
  void _mudarDia(int dias) { setState(() => _dataSelecionada = _dataSelecionada.add(Duration(days: dias))); }
  ...
  int _getJanelaVigiliaMinutos(int meses) { 
    if (meses < 1) return 60;
    if (meses < 3) return 90;
    if (meses < 5) return 120;
    if (meses < 8) return 150;
    if (meses < 12) return 210;
    if (meses < 18) return 300;
    return 360;
  }

  // --- MÉTODOS DE FEEDBACK (SNACKBAR) - INCLUÍDOS AGORA ---
  void _mostrarSucesso(String msg) { 
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg), backgroundColor: Colors.green, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)))); 
  }
  
  void _mostrarAviso(String msg) { 
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg), backgroundColor: Colors.orange, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)))); 
  }

  // --- AÇÕES DO USUÁRIO (COM CONTADORES) ---

  // 1. MAMADA
  void _acaoAmamentar(String lado) async {
    if (_globalService.dormindoNotifier.value) { _mostrarAviso("O bebê está dormindo! Acorde-o antes."); return; }
    
    if (_globalService.amamentandoNotifier.value) {
      if (_globalService.ladoMamadaNotifier.value == lado) { 
        _globalService.pararMamada(); 
        
        await ResumoService.atualizarContador(
          campo: 'mamadas', 
          valor: 1, 
  ...
  void _acaoDormir() async {
    if (_globalService.amamentandoNotifier.value) { _mostrarAviso("Pare a mamada antes de iniciar o sono."); return; }
  ...
  void _deletarRegistro(String id, Map<String, dynamic> dados) async { 
    ...
    if (ref != null) { 
      await ref.collection('rotina').doc(id).delete(); 
      
      try {
        if (dados['data'] != null) {
          DateTime dataEvento = DateTime.parse(dados['data']);
          String tipo = dados['tipo'];
          
          if (tipo == 'fralda') await ResumoService.atualizarContador(campo: 'fraldas', valor: -1, dataEvento: dataEvento);
          else if (tipo == 'mamada') await ResumoService.atualizarContador(campo: 'mamadas', valor: -1, dataEvento: dataEvento);
          else if (tipo == 'mamadeira') await ResumoService.atualizarContador(campo: 'mamadeiras', valor: -1, dataEvento: dataEvento);
          else if (tipo == 'banho') await ResumoService.atualizarContador(campo: 'banhos', valor: -1, dataEvento: dataEvento);
          else if (tipo == 'sono') {
             int dur = dados['duracao_segundos'] ?? 0;
             await ResumoService.atualizarContador(campo: 'sonecas', valor: -1, dataEvento: dataEvento);
             if (dur > 0) await ResumoService.atualizarContador(campo: 'sono_minutos', valor: -(dur ~/ 60), dataEvento: dataEvento);
          }
        }
      } catch (e) {
        debugPrint("Erro ao decrementar: $e");
      }
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(width: double.infinity, height: 55, child: ElevatedButton(onPressed: () async { 
                    ...
                    final ref = await BebeService.getRefBebeAtivo(); 
                    if (ref != null) { 
                      await ref.collection('rotina').add({ 'tipo': 'sono', 'duracao_segundos': duracao, 'duracao_fmt': "$min min", 'data': _inicioManual.toIso8601String(), 'fim': _fimManual.toIso8601String(), 'criado_em': FieldValue.serverTimestamp(), 'manual': true }); 
                      
                      await ResumoService.atualizarSono(minutos: min, data: _inicioManual);
                    } 
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Map<String, dynamic>?>(
      ...
      builder: (context, snapshotBebe) {
        ...
        int janelaMinutos = _getJanelaVigiliaMinutos(mesesVida);
                          ...
                          Center(
                            child: AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                gradient: isActive 
                                  ? LinearGradient(colors: [corTema, corTema.withOpacity(0.8)], begin: Alignment.topLeft, end: Alignment.bottomRight) 
                                  : const LinearGradient(colors: [Colors.white, Colors.white]),
                                borderRadius: BorderRadius.circular(30),
                                boxShadow: [
                                  BoxShadow(
                                    color: isActive ? corTema.withOpacity(0.4) : Colors.grey.withOpacity(0.05),
                                    blurRadius: 25,
                              ...
                              child: Column(
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      if (isActive)
                                        ScaleTransition(scale: _pulseController, child: Icon(statusIcon, color: Colors.white.withOpacity(0.8), size: 24)),
                                      const SizedBox(width: 10),
                                  ...
                                  if (isActive) ...[
                                    ...
                                    SizedBox(
                                      ...
                                      child: ElevatedButton(
                                        onPressed: amamentando ? _globalService.pararMamada : _globalService.pararSono,
                                  ...
                                  children: [
                                    ...
                                    GestureDetector(
                                      ...
                                      child: Container(
                                        ...
                                        child: Row(children: const [Icon(Icons.bathtub_outlined, size: 16, color: Colors.cyan), SizedBox(width: 5), Text("Banho", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.cyan))]),
                                      ),
                                    )
                                  ],
                                ),
  ...
  // --- WIDGETS AUXILIARES (Timeline, Botões, etc.) ---
      ...
      return StreamBuilder<QuerySnapshot>(stream: snapRef.data!.collection('rotina').orderBy('data', descending: true).limit(100).snapshots(), builder: (context, snapshot) {
        ...
        for (var doc in snapshot.data!.docs) {
          ...
          if (inicio.isBefore(fimDia.add(const Duration(hours: 12))) && (d['fim'] != null ? DateTime.parse(d['fim']).isAfter(inicioDia.subtract(const Duration(hours: 12))) : inicio.isAfter(inicioDia.subtract(const Duration(hours: 12))))) {
            if (tipo == 'sono') {
                ...
                elementos.add(Align(alignment: Alignment((pos * 2) - 1, yAlign), child: Container(padding: const EdgeInsets.all(2), decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 2)]), child: Icon(iconData, size: 10, color: corIcon))));
            }
          }
        }
        if (_isHoje && !isDormindoAgora && ultimoAcordarHoje != null) {
           ...
           if (inicioPrev.isBefore(fimDia)) {
              ...
              double w = pe - ps;
              if (w > 0) { elementos.add(Align(alignment: Alignment.centerLeft, child: FractionallySizedBox(widthFactor: ps + w, child: Container(alignment: Alignment.centerRight, child: FractionallySizedBox(widthFactor: w / (ps + w), child: Container(height: 24, decoration: BoxDecoration(color: Colors.purple.withOpacity(0.15), borderRadius: BorderRadius.circular(6), border: Border.all(color: Colors.purple.withOpacity(0.3), width: 1, style: BorderStyle.solid)))))))); }
           }
        }
        if (_isHoje) { double posAgora = getPos(DateTime.now()); elementos.add(Align(alignment: Alignment((posAgora * 2) - 1, 0), child: Container(width: 2, height: 40, color: Colors.redAccent.withOpacity(0.6)))); }
        return Container(height: 70, width: double.infinity, margin: const EdgeInsets.symmetric(vertical: 5), child: Stack(alignment: Alignment.center, clipBehavior: Clip.none, children: [...List.generate(25, (h) { double pos = (h * 60) / 1440.0; bool showLabel = h % 2 == 0; return Positioned(left: pos * (MediaQuery.of(context).size.width - 96) + 10, bottom: -15, child: Column(children: [Container(height: showLabel ? 8 : 4, width: 1, color: Colors.grey.withOpacity(0.3)), if (showLabel) Text("$h", style: TextStyle(fontSize: 10, color: Colors.grey.shade400, fontWeight: FontWeight.bold))])); }), ...elementos]));
      });
    });
  }
  ...
  Widget _buildBotaoPeito(String lado, String label, bool amamentando, String ladoAtivo) {
    ...
    return GestureDetector(onTap: isDisabled ? null : () => _acaoAmamentar(lado), child: AnimatedContainer(duration: const Duration(milliseconds: 300), height: 100, decoration: BoxDecoration(color: isAtivo ? const Color(0xFFEC407A) : Colors.white, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: isAtivo ? const Color(0xFFEC407A).withOpacity(0.4) : Colors.grey.withOpacity(0.05), blurRadius: 15, offset: const Offset(0, 5))], border: Border.all(color: isAtivo ? Colors.transparent : Colors.pink.shade50)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(isAtivo ? Icons.pause_circle_filled : Icons.play_circle_fill_rounded, color: isAtivo ? Colors.white : Colors.pink.shade200, size: 32), const SizedBox(height: 8), Text(label, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isAtivo ? Colors.white : Colors.grey.shade700))])));
  }

  Widget _buildBotaoLargo(String label, IconData icon, Color cor, VoidCallback onTap, {bool isPrimary = false}) {
    return GestureDetector(onTap: onTap, child: Container(height: 70, width: double.infinity, decoration: BoxDecoration(color: isPrimary ? cor : Colors.white, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: cor.withOpacity(isPrimary ? 0.3 : 0.05), blurRadius: 15, offset: const Offset(0, 5))], border: isPrimary ? null : Border.all(color: cor.withOpacity(0.2))), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, color: isPrimary ? Colors.white : cor, size: 26), const SizedBox(width: 15), Text(label, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isPrimary ? Colors.white : cor))])));
  }
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.symmetric(horizontal: 24), itemCount: docs.length,
          itemBuilder: (ctx, i) {
            final d = docs[i].data() as Map<String, dynamic>;
            DateTime dt; try { dt = DateTime.parse(d['data']); } catch (e) { dt = DateTime.now(); }
            final hora = DateFormat('HH:mm').format(dt);
            
            Color cor = Colors.grey; IconData icone = Icons.circle;
            if (d['tipo'] == 'mamada') { cor = const Color(0xFFEC407A); icone = Icons.favorite_rounded; }
            else if (d['tipo'] == 'sono') { cor = const Color(0xFF3949AB); icone = Icons.bedtime_rounded; }
            else if (d['tipo'] == 'fralda') { cor = Colors.teal; icone = Icons.layers_rounded; }
            else if (d['tipo'] == 'mamadeira') { cor = Colors.orange; icone = Icons.local_drink_rounded; }
            else if (d['tipo'] == 'banho') { cor = Colors.cyan; icone = Icons.bathtub_rounded; }
            ...
            return Padding(
              ...
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  const SizedBox(width: 15),
                  Column(children: [Container(width: 12, height: 12, decoration: BoxDecoration(color: cor, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2), boxShadow: [BoxShadow(color: cor.withOpacity(0.4), blurRadius: 6)])), if (i != docs.length - 1) Container(width: 2, height: 40, color: Colors.grey.shade100)]),
                  const SizedBox(width: 15),
                  Expanded(child: GestureDetector(onTap: () => _editarRegistro(docs[i].id, d), child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(12)), child: Row(children: [Icon(icone, size: 18, color: cor), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(d['tipo'].toString().toUpperCase(), style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey.shade500)), Text(d['duracao_fmt'] ?? d['conteudo'] ?? "${d['ml']} ml" ?? "", style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.black87))])), GestureDetector(onTap: () => _deletarRegistro(docs[i].id, d), child: Icon(Icons.close_rounded, size: 16, color: Colors.grey.shade300))]))))
              ]),
            );
          },
        );
